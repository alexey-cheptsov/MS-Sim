...
/*VS*/      VS_S_, VS_R_, VS_L_,
    ...
    // sets value of all q-approx_elements
    void set_R_reg_VS(float* values /*[n]*/) {
        // preparation of buffer
        proxy_clear(Ports_QQmt::set_R_reg_VS);

        for (int i=0; i<N_VS; i++) {
            add_proxy_value<float>(Ports_QQmt::set_R_reg_VS, values[i]);
        }
        proxy_flush_collective_spread(Ports_QQmt::set_R_reg_VS);
        proxy_clear(Ports_QQmt::set_R_reg_VS);

        add_proxy_value<int>(Ports_QQmt::command_flow, Commands_QQmt::set_R_reg_VS);
        proxy_flush_collective_replicate(Ports_QQmt::command_flow);
        proxy_clear(Ports_QQmt::command_flow);
    }
	...
	for (int i=0; i<N_OS; i++) {
	    ...
	    q_OS_old[i] = start_q_OS;
	}
	set_Q_OS(q_OS);
	
	for (int i=0; i<N_Streb; i++) {
	    q_Streb[i] = start_q_Streb;
	    
	    q_Streb_old[i] = start_q_Streb;
	}
	set_Q_Streb(q_Streb);
	...
	for (int i=0; i<N_AM; i++) {
	    ...
	    q_AM_old[i] = start_q_AM;
	    qm_AM_old[i] = start_q_AM;
	}
	set_Q_AM(q_AM);
	set_Qm_AM(qm_AM);
	set_Qm0_AM(qm0_AM);
	...
	for (int i=0; i<N_VS; i++) {
	    ...
	    q_VS_old[i] = start_q_VS;
	    qm_VS_old[i] = Qm0;
	}
	qms_VS[0] = start_qms_VS;
	set_Q_VS(q_VS);
	set_Qms_VS(qms_VS);
	...
	//
        ...
        //
        float r_reg_start = 0.0;
        float r_reg_end = 25.0;
        float r_reg_delta = 5;

        float r_reg = r_reg_start;
...
int main (int argc, char* argv[]) {
    ...
    float AM_V = 8000.0; 	// specific volume of FR per discrete element
    float AM_Qm0 = 0.0175;
