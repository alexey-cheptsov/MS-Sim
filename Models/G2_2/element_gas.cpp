...
public:
    ...
    int proxy_disp_p = 13; // proxy displacement of p- nodes
                           // (contiguous numbering of proxy-ports)

    Qmt** QQ; // underlying services
    p**   PP;
	...
	QQ[0] = new Qmt(1/*id*/, "Qmt0", communicator_,
	...
	//
        add_proxy(new LocalIntBuffer  (id, id_str,  Ports_Qmt::command_flow,   communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_Q,          communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_Q,          communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_Qm,         communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_Qm,         communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_Qs,         communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_Qs,         communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_Qms,        communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_Qms,        communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_R_reg,      communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_R_reg,      communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::set_P,          communicator));
        add_proxy(new LocalFloatBuffer(id, id_str,  Ports_Qmt::get_P,          communicator));
	...
	for (int i=0; i<1; i++) 
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::command_flow  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::command_flow));
	...
	for (int i=0; i<2; i++) 
        ...
        // Data Flow Master->Q
	for (int i=0; i<1; i++) {
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_Q  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_Q,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_Qm  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_Qm,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_Qs  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_Qs,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_Qms  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_Qms,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_R_reg  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_R_reg,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink(master /*snd_id*/, Ports_Qmt::set_P  + proxy_disp /*port*/, 
						 "Qmt" + to_string(i) /*rcv_id*/, Ports_Qmt::set_P,
						 n-1 /*size*/));
	}
	...
	// Data Flow Q->Master
        for (int i=0; i<1; i++) {
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_Q,
						 master /*rcv_id*/, Ports_Qmt::get_Q + proxy_disp  /*port*/,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_Qm,
						 master /*rcv_id*/, Ports_Qmt::get_Qm + proxy_disp  /*port*/,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_Qs,
						 master /*rcv_id*/, Ports_Qmt::get_Qs + proxy_disp  /*port*/,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_Qms,
						 master /*rcv_id*/, Ports_Qmt::get_Qms + proxy_disp  /*port*/,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_R_reg,
						 master /*rcv_id*/, Ports_Qmt::get_R_reg + proxy_disp  /*port*/,
						 n /*size*/));
	    communicator->add_comm_link(new CommLink("Qmt" + to_string(i) /*snd_id*/, Ports_Qmt::get_P,
						 master /*rcv_id*/, Ports_Qmt::get_P + proxy_disp  /*port*/,
						 n-1 /*size*/));
	}
	...
	// Data Flow P->Master
        ...
        // Data Flow Q->P
        for (int i=0; i<1; i++) { 
	    communicator->add_comm_link(new CommLink("Qmt"+to_string(i)+"_q0"/*snd_id*/, Ports_q::num_get_q, 
						 "P"+to_string(i) /*rcv_id*/,	Ports_p::num_set_qout));
						 
	    communicator->add_comm_link(new CommLink("Qmt"+to_string(i)+"_q"+to_string(n-1)/*snd_id*/, Ports_q::num_get_q, 
	...
	for (int i=0; i<1; i++) { 
	    communicator->add_comm_link(new CommLink("P"+to_string(i)/*snd_id*/, Ports_p::num_get_p, 
						 "Qmt"+to_string(i)+"_q0" /*rcv_id*/, Ports_q::num_set_pstart));
						 
	    communicator->add_comm_link(new CommLink("P"+to_string(i+1)/*snd_id*/, Ports_p::num_get_p, 
						 "Qmt"+to_string(i)+"_q"+to_string(n-1)/*rcv_id*/, Ports_q::num_set_pend));
        }
	...
	proxy_clear(Ports_Qmt::set_Q);
	
	for (int i=0; i<n; i++) {
	    add_proxy_value<float>(Ports_Qmt::set_Q, values[i]);
	}
	proxy_flush_collective_spread(Ports_Qmt::set_Q);
	proxy_clear(Ports_Qmt::set_Q);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_Q);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }

    // sets value of all q-approx_elements
    void set_Qm(float* values /*[n]*/) {
	// preparation of buffer
	proxy_clear(Ports_Qmt::set_Qm);
	
	for (int i=0; i<n; i++) {
	    add_proxy_value<float>(Ports_Qmt::set_Qm, values[i]);
	}
	proxy_flush_collective_spread(Ports_Qmt::set_Qm);
	proxy_clear(Ports_Qmt::set_Qm);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_Qm);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }

    // sets value of all q-approx_elements
    void set_Qs(float* values /*[n]*/) {
	// preparation of buffer
	proxy_clear(Ports_Qmt::set_Qs);
	
	for (int i=0; i<n; i++) {
	    add_proxy_value<float>(Ports_Qmt::set_Qs, values[i]);
	}
	proxy_flush_collective_spread(Ports_Qmt::set_Qs);
	proxy_clear(Ports_Qmt::set_Qs);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_Qs);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }
    
    // sets value of all q-approx_elements
    void set_Qms(float* values /*[n]*/) {
	// preparation of buffer
	proxy_clear(Ports_Qmt::set_Qms);
	
	for (int i=0; i<n; i++) {
	    add_proxy_value<float>(Ports_Qmt::set_Qms, values[i]);
	}
	proxy_flush_collective_spread(Ports_Qmt::set_Qms);
	proxy_clear(Ports_Qmt::set_Qms);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_Qms);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }
    
    // sets value of all q-approx_elements
    void set_R_reg(float* values /*[n]*/) {
	// preparation of buffer
	proxy_clear(Ports_Qmt::set_R_reg);
	
	for (int i=0; i<n; i++) {
	    add_proxy_value<float>(Ports_Qmt::set_R_reg, values[i]);
	}
	proxy_flush_collective_spread(Ports_Qmt::set_R_reg);
	proxy_clear(Ports_Qmt::set_R_reg);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_R_reg);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }
	...
	proxy_clear(Ports_Qmt::set_P);

	for (int i=0; i<n-1; i++)
	    add_proxy_value<float>(Ports_Qmt::set_P, values[i]);
	proxy_flush_collective_spread(Ports_Qmt::set_P);
	proxy_clear(Ports_Qmt::set_P);
	
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::set_P);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);
    }
	...
	proxy_clear(Ports_Qmt::get_Q);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_Q);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_Q);
	for (int i=0; i<n; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_Q, i);
	proxy_clear(Ports_Qmt::get_Q);
    }
    
    // Gets q of q-approx_elements
    void get_Qm(float* values  /*[n]*/) {
	proxy_clear(Ports_Qmt::get_Qm);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_Qm);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_Qm);
	for (int i=0; i<n; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_Qm, i);
	proxy_clear(Ports_Qmt::get_Qs);
    }

    // Gets q of q-approx_elements
    void get_Qs(float* values  /*[n]*/) {
	proxy_clear(Ports_Qmt::get_Qs);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_Qs);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_Qs);
	for (int i=0; i<n; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_Qs, i);
	proxy_clear(Ports_Qmt::get_Qs);
    }

    // Gets q of q-approx_elements
    void get_Qms(float* values  /*[n]*/) {
	proxy_clear(Ports_Qmt::get_Qms);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_Qms);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_Qms);
	for (int i=0; i<n; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_Qms, i);
	proxy_clear(Ports_Qmt::get_Qms);
    }
    
        // Gets q of q-approx_elements
    void get_R_reg(float* values  /*[n]*/) {
	proxy_clear(Ports_Qmt::get_R_reg);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_R_reg);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_R_reg);
	for (int i=0; i<n; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_R_reg, i);
	proxy_clear(Ports_Qmt::get_R_reg);
    }

    // Gets p of q-approx_elements
    void get_P(float* values  /*[n-1]*/) {
	proxy_clear(Ports_Qmt::get_P);
    
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::get_P);
	proxy_flush_collective_replicate(Ports_Qmt::command_flow);
	proxy_clear(Ports_Qmt::command_flow);	
	
	proxy_sync(Ports_Qmt::get_P);
	for (int i=0; i<n-1; i++)
	    values[i] = get_proxy_value<float>(Ports_Qmt::get_P, i);
	proxy_clear(Ports_Qmt::get_Q);
    }

    // inits time stamp for all ms
    void init_time() {
	add_proxy_value(Ports_Qmt::command_flow, Commands_Qmt::init_time/*value*/);
        proxy_flush_collective_replicate(Ports_Qmt::command_flow);
        proxy_clear(Ports_Qmt::command_flow);
	...
	add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::simulation/*value*/);
        proxy_flush_collective_replicate(Ports_Qmt::command_flow);
        proxy_clear(Ports_Qmt::command_flow);
...
//	for (int i=0; i<1; i++) {
	...
	// Stopping the worker-ms
        add_proxy_value<int>(Ports_Qmt::command_flow, Commands_Qmt::stop);
        proxy_flush_collective_replicate(Ports_Qmt::command_flow);
        proxy_clear(Ports_Qmt::command_flow);
...
int main (int argc, char* argv[]) {
    ...
    mpi_map.add({"P0",        2});
    mpi_map.add({"P1",        3});    
    mpi_map.add({"Qmt0_q0",   4});
    mpi_map.add({"Qmt0_qmt0", 4});
    mpi_map.add({"Qmt0_q1",   5});
    mpi_map.add({"Qmt0_qmt1", 5});
    mpi_map.add({"Qmt0_q2",   6});
    mpi_map.add({"Qmt0_qmt2", 6});
    mpi_map.add({"Qmt0_q3",   7});
    mpi_map.add({"Qmt0_qmt3", 7});
    mpi_map.add({"Qmt0_q4",   8});
    mpi_map.add({"Qmt0_qmt4", 8});
    mpi_map.add({"Qmt0_q5",   9});
    mpi_map.add({"Qmt0_qmt5", 9});
    mpi_map.add({"Qmt0_q6",  10});
    mpi_map.add({"Qmt0_qmt6",10});
    mpi_map.add({"Qmt0_q7",  11});
    mpi_map.add({"Qmt0_qmt7",11});
    mpi_map.add({"Qmt0_q8",  12});
    mpi_map.add({"Qmt0_qmt8",12});
    mpi_map.add({"Qmt0_q9",  13});
    mpi_map.add({"Qmt0_qmt9",13});
    mpi_map.add({"Qmt0_p0",  14});
    mpi_map.add({"Qmt0_p1",  15});
    mpi_map.add({"Qmt0_p2",  16});
    mpi_map.add({"Qmt0_p3",  17});
    mpi_map.add({"Qmt0_p4",  18});
    mpi_map.add({"Qmt0_p5",  19});
    mpi_map.add({"Qmt0_p6",  20});
    mpi_map.add({"Qmt0_p7",  21});
    mpi_map.add({"Qmt0_p8",  22});
    ...
    deployment_pool.add_ms(model_G2_2);
    deployment_pool.deploy_all();
    deployment_pool.join_all();
