...
/*VS*/      VS_S_, VS_R_, VS_L_,
    ...
    void save() {
	add_proxy_value(Ports_QQmt::command_flow, Commands_QQmt::save/*value*/);
        proxy_flush_collective_replicate(Ports_QQmt::command_flow);
        proxy_clear(Ports_QQmt::command_flow);

        add_proxy_value(proxy_disp_p + Ports_p::command_flow, Commands_p::save /*value*/);
        proxy_flush_collective_replicate(proxy_disp_p + Ports_p::command_flow);
        proxy_clear(proxy_disp_p + Ports_p::command_flow);
    }
	...
	cout << "Now sleeping for 20 minutes: ";
    	...
    	while ( num_step < (nr_of_additional_minutes * 60 / (solv_params.nr_num_steps * solv_params.time_step)) ) {
    	    ...
    	    cout << "============== Additional Iteration " << num_step << "==============" << endl;
    	    simulation_step();
